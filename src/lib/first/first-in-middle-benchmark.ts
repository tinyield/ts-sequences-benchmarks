...
/**
 * FirstInMiddleBenchmark
 * Benchmarks the usage of the `findFirst()` operator.
 * The match element is found in the middle of the sequence.
 * <p>
 * Pipeline:
 * Sequence.of(EVEN, EVEN,...,ODD, ..., EVEN)
 * .filter(isOdd)
 * .findFirst()
 */
export class FirstInMiddleBenchmark implements Benchmark {
    /**
     ...
     */
    name(): string {
        return `First in the Middle ${getCollectionSizeLabel()}`;
    }
