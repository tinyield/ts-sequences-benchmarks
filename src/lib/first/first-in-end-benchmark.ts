 ...
 * The match element is found in the last element.
 * <p>
 * Pipeline:
 * Sequence.of(EVEN, EVEN, ..., ODD)
 * .filter(isOdd)
 * .findFirst()
 */
export class FirstInEndBenchmark implements Benchmark {
    /**
    ...
    /**
     * Prepares an array with a single ODD element in the end.
     *
     ...
     */
    name(): string {
        return `First in the End ${getCollectionSizeLabel()}`;
    }
